...
| 1.48.0 | **Par levels → auto-reorder draft POs: inventory items gain a **par level** (target reorder-up-to quantity; `min_quantity` stays the trigger). A new Auto-Reorder Suggestions panel (owner & manager, on Supply Orders) lists everything below its reorder point with a suggested quantity that builds back to par + estimated cost; the user reviews/edits, optionally picks a vendor, and creates pending purchase orders in one click. New `inventory.par_level` column and `GET /inventory/reorder-suggestions` + `POST /inventory/reorder/create`; par level editable on the inventory screen.** |
| 1.47.0 | **Live labor + overtime alerts: a Live Labor board (owner & manager, on Sales Analytics) shows who's on the clock now with shift length, hours today/this-week, and labor cost — plus headcount, labor cost, net sales, **labor %** (color-coded), and sales-per-labor-hour, auto-refreshing. Weekly hours drive **approaching-OT (≥36h)** and **overtime (≥40h)** flags with a summary alert banner; continuous shifts over 8h flag as long shifts. New `GET /api/reports/labor` (computed from clock records + payments; no new tables).** |
| 1.46.0 | **End-of-day close: Z-report + cash reconciliation. Owners/managers run a per-location, per-day Z-report (orders, gross/net sales, discounts, comps, service, tax, tips, total collected, refunds, voids, tendered-by-method) — printable. Plus full cash-drawer reconciliation: open with a float, record pay-ins/pay-outs, track cash sales, then count & close for over/short with a bank deposit and a history of closes. New `/api/reports` endpoints; `cash_drawers` + `cash_events` tables; surfaced on the Sales Analytics view for both roles.** |
